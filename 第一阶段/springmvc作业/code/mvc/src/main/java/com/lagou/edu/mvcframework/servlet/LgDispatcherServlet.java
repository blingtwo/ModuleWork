...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            Set<String> anthoritySet = new HashSet<>();
            //获取@Security注释类上的value值
            if (aClass.isAnnotationPresent(Security.class)) {
                Security annotation = aClass.getAnnotation(Security.class);
                String[] userValues = annotation.value();
                anthoritySet.addAll(Arrays.asList(userValues));
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                //获取@Security注释方法上的value值
                Security security = method.getAnnotation(Security.class);
                if (security != null) {
                    String[] values = security.value();
                    anthoritySet.addAll(Arrays.asList(values));
                }

                //将anthority列表保存到handler.anthoritySet中
                Set<String> getAnthoritySet = handler.getAnthoritySet();
                if (anthoritySet.size() > 0) {
                    getAnthoritySet.addAll(anthoritySet);
                }
        ...
        try{
            ...
            for (int i = 0; i < classNames.size(); i++) {
                ...
                if(aClass.isAnnotationPresent(LagouController.class)) {
                    // controll处er的id此不做过多处理，不取value了，就拿类的首字母小写作为id，保存到ioc中
...
//        method.invoke() //
        ...
        if (!checkAuthority(req,handler)) {
            resp.getWriter().write("401 not authorized! "+"username:" + req.getParameter("username") + " no authorized!");
        } else {
            resp.getWriter().write("username:"+ req.getParameter("username") + "  pass!");
        }
    ...
    // 检查用户权限，username是否包含在@Security注解中
    private boolean checkAuthority(HttpServletRequest req, Handler handler) {
        Set<String> anthoritySet = handler.getAnthoritySet();
        if (anthoritySet.size() <=0) { return false; }

        String username = req.getParameter("username");
        for (String user : anthoritySet) {
